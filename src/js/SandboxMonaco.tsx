...
export default function SandboxMonaco() {
    ...
    <div className="card sandbox-wrap" style={{ marginTop: 0 }}>
      <div className="sandbox-header">
        ...
        <div className="sandbox-actions">
          <button type="button" onClick={run}>▶ {ui.js.sandbox.runButton}</button>
          <button type="button" className="secondary" onClick={() => setOutput([])}>{ui.js.sandbox.clearOutput}</button>
          <button type="button" className="secondary" onClick={() => { editorRef.current?.setValue(DEFAULT_CODE); setOutput([]) }}>{ui.js.sandbox.reset}</button>
        </div>
      </div>
