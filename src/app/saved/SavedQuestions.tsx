...
export default function SavedQuestions() {
  ...
  // Slugs are disambiguated against the whole catalog, so links must be derived
  // from it too — deriving them from the saved subset would drop the suffix.
  const { questions: fullCatalog } = useQuestionCatalog()
  const permalinks = useMemo(() => questionPathById(fullCatalog), [fullCatalog])
            ...
            {savedQuestions.map(q => (
              <article key={q.id} className="q-card q-card--editorial">
                ...
                <div style={{ marginTop: '0.75rem' }}>
                  <Link
                    href={permalinks.get(q.id) ?? `${PATH_FOR_PAGE.questions}?q=${encodeURIComponent(q.title)}`}
